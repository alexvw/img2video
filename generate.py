...
initial_alpha = 0.22 #how hard to influence the first frame. 0.23 is a pretty good default value
ratio = 0.7 #how much to influence each frame. is multiplied against previous alpha, 0.8 works alright


x = model(text, text_neg, 0.32, 0.5, image_path)
save_webm(x, 'output', 'turtle-0.32-0.5.webm', 10) #x, folder, filename, fps

x = model(text, text_neg, 0.25, 0.5, image_path)
save_webm(x, 'output', 'turtle-0.25-0.5.webm', 10) #x, folder, filename, fps

x = model(text, text_neg, 0.23, 0.5, image_path)
save_webm(x, 'output', 'turtle-0.23-0.5.webm', 10) #x, folder, filename, fps

x = model(text, text_neg, 0.21, 0.5, image_path)
save_webm(x, 'output', 'turtle-0.21-0.5.webm', 10) #x, folder, filename, fps

x = model(text, text_neg, 0.16, 0.5, image_path)
save_webm(x, 'output', 'turtle-0.16-0.5.webm', 10) #x, folder, filename, fps
