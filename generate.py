...
text_neg = 'watermark'
initial_alpha = 0.23 #how hard to influence the first frame. 0.23 is a pretty good default value
...
image_path = 'input.png' #path from the root to where the initial frame image is

model = SDVideo(model_path, device)
x = model(text, text_neg, initial_alpha, ratio, image_path)

# save_vid(x, 'output')
save_webm(x, 'output', 'output.webm', 10) #x, folder, filename, fps

#to run this, change parameters above and run "python ./generate.py"